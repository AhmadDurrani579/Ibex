...
- (void)initValues
{
    ...
    [_controllerScrollView setTag:101];
    [_menuScrollView setTag:102];
...
- (void)configureUserInterface
{
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        UIImageView *tempImage = [[UIImageView alloc] initWithFrame:menuItemFrame];

        
        if(index==0){
            [tempImage setImage:[UIImage imageNamed:@"tabVehicleTrakking"]];
        }
        else{
            [tempImage setImage:[UIImage imageNamed:@"tabPersonalTrakking"]];
        }

        [tempImage setContentMode:UIViewContentModeScaleAspectFit];
        [_menuScrollView addSubview:tempImage];
    ...
    [_menuScrollView sendSubviewToBack:[_menuScrollView.subviews lastObject]];
...
- (void)moveToPageWithOutAnim:(NSInteger)index
{
    if (index >= 0 && index < _controllerArray.count) {
        // Update page if changed
